...
export default function WinnerModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 transition-opacity duration-300">
      <div
        className={`w-full max-w-md rounded-2xl border border-white/10 bg-[#111827] p-6 shadow-2xl transition-all duration-300 ${
      ...
      >
        ...
        {isSeller && winnerCustomerId ? (
          <p className="mt-4 text-sm text-slate-300">Winning Bidder: {winnerCustomerId}</p>
        ) : null}

        <p className="mt-3 text-sm text-slate-300">
          {isSeller ? "Amount credited to your wallet" : "Amount deducted from wallet"}
        </p>
